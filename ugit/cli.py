...
def parse_args():
    ...
    diff_parser.set_defaults(func=diff_)
...
def k(args):
    ...
    for name, ref in data.iter_refs(deref=False):
        dot += f'"{name}" [shape="note"]\n'
        dot += f'"{name}" -> "{ref.value}"\n'
...
def status(args):
    ...
    for path, action in diff.iter_changed_files(head_tree, base.get_index_tree()):
